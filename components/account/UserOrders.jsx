...
export default function UserOrdersPage({ orders }) {
  ...
  return (
    <div className="mx-auto max-w-full p-4 xl:p-0">
      <h1 className="text-umbra-100 mb-6 text-[24px] leading-[130%] font-normal">{t('yourOrders')}</h1>
      ...
      {orders.length === 0 ? (
        <div className="text-muted-foreground flex max-h-[80dvh] min-h-[300px] flex-col items-center justify-center rounded border text-center">
          ...
          <p className="text-sm text-gray-500">{t('orderNotFoundMessage')}</p>
          <div className="mt-5">
            <Link
              href={`/shop`}
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-background after:bg-border sticky top-0 after:absolute after:inset-x-0 after:bottom-0 after:h-px after:content-[''] [&>*]:whitespace-nowrap">
                <TableHead className="text-center font-sans font-normal">{t('orderId')}</TableHead>
                <TableHead className="text-center font-sans font-normal">{t('orderDate')}</TableHead>
                <TableHead className="text-center font-sans font-normal">{t('quantity')}</TableHead>
                <TableHead className="text-center font-sans font-normal">{t('totalAmount')}</TableHead>
                <TableHead className="text-center font-sans font-normal">{t('status')}</TableHead>
                ...
                <TableHead className="text-center font-sans font-normal">{t('details')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {orders.map((order) => (
