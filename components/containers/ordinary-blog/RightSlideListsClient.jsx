...
const RightSlideListsClient = ({ blogs, blogId }) => {
  const t = useTranslations('BlogPage');

  // Check if we have any similar posts after filtering
  if (!blogs || blogs.length === 0) {
    return (
      <div>
        <div className="mb-5">
          <h5 className="text-umbra-100 font-sans text-[22px] leading-[130%] font-normal tracking-normal">
            {t('similarReads.title')}
          </h5>
        </div>
        <div className="py-8 text-center">
          <p className="text-gray-600">{t('similarReads.noSimilarPosts')}</p>
        </div>
      </div>
    );
  }
...
export default RightSlideListsClient;
