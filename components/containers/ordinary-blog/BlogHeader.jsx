      ...
      <div className="flex items-end justify-between pb-6 xl:pb-[80px]">
        <div className="flex w-full flex-col gap-4 xl:max-w-[70%]">
          <div className="flex flex-wrap items-center gap-2">
            <button className="main-button-black border-umbra-100 !xl:text-[16px] rounded-md border-1 px-2 py-1 !text-[14px] xl:px-4 xl:py-2">
              {blogData.status}
            </button>
            {blogData.tags && (
              <>
                {blogData?.tags?.split(',').map((tag, idx) => (
                  <Badge
                    variant="outline"
                    key={idx}
                    className="!bg-white-100 border-umbra-100 h-[30px] rounded-sm border-1 px-2 py-0 !text-[12px] font-normal xl:h-[38px] xl:px-4"
                  >
                    {tag.trim()}
                  </Badge>
                ))}
      ...
      <div className="relative h-[400px] w-full xl:h-[631px] xl:w-[1280px]">
        {blogData.image && (
          <Image
            ...
            width={1280}
            height={631}
            className="h-[400px] w-full object-cover xl:h-[631px] xl:w-[1280px]"
          />
        )}
        {/* Left white box */}
