...
const OrderFailedEmail = ({ orderData, rejectionReason }) => {
  if (!orderData) {
    return (
      <Html>
        <Head />
  ...
  const defaultRejectionReason =
    rejectionReason ||
    'Your order could not be processed at this time. Please contact our support team for assistance.';
  ...
  return (
    <Html>
      ...
      <Font
        ...
        fontStyle="normal"
      />
      <Preview>Order Failed - We're sorry, your order could not be processed</Preview>
      <Tailwind
        config={{
          theme: {
            extend: {
        ...
        <Body className="bg-gray-50 p-6">
          <Container className="rounded-lg bg-white p-6 shadow-md">
            ...
            <Section className="rounded-md bg-red-50 p-4">
              <Text className="mb-2 text-center text-2xl font-bold text-red-800">Order Failed</Text>
              <Text className="m-0 text-center text-red-700">
                We're sorry, {customer_name}. Your order could not be processed.
              </Text>
            </Section>

            <Text className="mb-4 text-lg">Hi, {customer_name}</Text>
            <Text className="mb-4 leading-relaxed text-gray-700">
              We're sorry to inform you that your order could not be processed successfully.
            </Text>

            <Section className="mb-6 rounded-lg bg-orange-50 p-4">
              <Text className="m-0 font-semibold text-orange-800">Reason for Failure:</Text>
              <Text className="m-0 text-orange-700">{defaultRejectionReason}</Text>
            </Section>

            <Section className="rounded-lg bg-red-50 p-4">
              <Row>
                <Column>
                  <Text className="m-0 font-semibold text-red-800">Order Number</Text>
                  <Text className="text-uppercase m-0 font-mono text-red-700">{code}</Text>
                </Column>
                <Column className="text-right">
                  <Text className="m-0 text-red-700">Order Date: {formatDate(created_at)}</Text>
                  <Text className="m-0 text-red-700">Status: Failed</Text>
                </Column>
              </Row>
            </Section>
            ...
            <Section className="mb-4">
              <Row className="mb-2">
                ...
                <Column className="w-1/4 text-center">
                  ...
                  <Text className="m-0 text-sm font-semibold text-red-600 capitalize">Failed</Text>
                </Column>
                <Column className="w-1/4 text-center">
                  <Text className="m-0 text-sm font-medium text-gray-500">Order Amount</Text>
                  <Text className="m-0 text-sm font-bold text-red-600">${formatPrice(total)}</Text>
                </Column>
              </Row>
            </Section>
              ...
              <Section className="mb-4">
                <Section className="mb-2">
                  <Row>
                    ...
                    <Column className="text-right">
                      <Text className="m-0 inline-block rounded-full bg-yellow-50 px-2 py-1 font-sans text-xs text-yellow-700 capitalize">
                        Regular
                      </Text>
                    </Column>
                  </Row>
                ...
                {products.map((item, index) => (
                  <Section
                    ...
                    className="mb-2 rounded-lg border border-yellow-100 bg-yellow-50/10 p-3"
                  >
                    <Row className="py-2">
                      <Column className="w-[50%]">
              ...
              <Section className="mb-4">
                <Section className="mb-2">
                  <Row>
                    ...
                    <Column className="text-right">
                      <Text className="m-0 inline-block rounded-full bg-orange-50 px-2 py-1 font-sans text-xs text-orange-700 capitalize">
                        Wholesale
                      </Text>
                    </Column>
                  </Row>
                ...
                {ws_products.map((item, index) => (
                  <Section
                    ...
                    className="mb-2 rounded-lg border border-orange-50 bg-orange-50/10 p-3"
                  >
                    <Row className="py-2">
                      <Column className="w-[50%]">
                        <Text className="m-0 font-semibold">
            ...
            <Section className="mt-6 text-right">
              ...
              <Row className="mt-3 rounded-md bg-red-50 p-2">
                ...
                <Column className="w-px" />
                <Column className="w-32 text-right">
                  <Text className="m-0 font-bold text-red-800">${formatPrice(total)}</Text>
                </Column>
              </Row>
            </Section>
            ...
            <Section className="text-center">
              ...
              <Text className="m-0 text-sm text-gray-700">
                We're here to help resolve any issues and ensure you have a great shopping experience.
              </Text>
