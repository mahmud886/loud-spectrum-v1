...
export default function ShopCategoryError({ error, reset }) {
    ...
    <div className="container mx-auto my-[100px]">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h1 className="mb-4 text-4xl font-bold text-gray-900">
          {t('categoryError.title', { defaultValue: 'Category Loading Error' })}
        </h1>

        <p className="mb-8 text-lg text-gray-600">
          {t('categoryError.description', {
            defaultValue:
              'We encountered an error while loading this product category. Please try again or contact support if the problem persists.',
          })}
        </p>
        ...
        <div className="flex flex-col justify-center gap-4 sm:flex-row">
          ...
          >
            {t('categoryError.tryAgain', { defaultValue: 'Try Again' })}
          </button>
          ...
          >
            {t('categoryError.backToShop', { defaultValue: 'Back to Shop' })}
          </Link>
        </div>
        ...
        <div className="mt-12 rounded-lg bg-gray-50 p-6">
          <h3 className="mb-3 text-lg font-medium text-gray-900">
            {t('categoryError.alternatives.title', { defaultValue: 'Explore Other Categories' })}
          </h3>
          <p className="mb-4 text-gray-600">
            {t('categoryError.alternatives.description', {
              defaultValue: 'While we fix this issue, you can explore other product categories.',
            })}
          </p>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            >
              {t('categoryError.alternatives.allProducts', { defaultValue: 'All Products' })}
            </Link>
            <Link
              href="/blog"
              className="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50"
            >
              {t('categoryError.alternatives.blog', { defaultValue: 'Read Blog' })}
            </Link>
            <Link
              href="/about"
              className="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50"
            >
              {t('categoryError.alternatives.about', { defaultValue: 'About Us' })}
            </Link>
            <Link
              href="/contact"
              className="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50"
            >
              {t('categoryError.alternatives.contact', { defaultValue: 'Contact Us' })}
            </Link>
          </div>
        </div>
      </div>
