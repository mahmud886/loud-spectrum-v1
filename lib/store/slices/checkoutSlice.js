...
import { selectSimplifiedCartProducts } from './cartSlice';
...
const initialState = {
  ...
  selectedCourier: '',
  shippingType: '',
  shippingTypeFormatted: '', // For DHL formatted strings like "dhl-N-0 - EXPRESS DOMESTIC"
...
const checkoutSlice = createSlice({
  ...
  reducers: {
    ...
    setShippingTypeFormatted: (state, { payload }) => {
      state.shippingTypeFormatted = payload;
    },
    ...
    setShippingCostToZero: (state) => {
      state.orderSummary.shipping = 0;
      state.orderSummary.total = calculateTotal(state.orderSummary);
    },
    ...
    setSubtotal: (state, { payload }) => {
      ...
      // Recalculate coupon discount based on updated subtotal if a coupon is applied
      if (state.discountCoupon.applied) {
        const { couponType, originalDiscountValue } = state.discountCoupon;
        let newDiscountAmount = 0;
        if (couponType === 'Fixed') {
          newDiscountAmount = Number(originalDiscountValue || 0);
        } else if (couponType === 'Percentage') {
          const percentage = Number(originalDiscountValue || 0);
          newDiscountAmount = (percentage / 100) * Number(state.orderSummary.subtotal || 0);
        }
        // Cap discount to not exceed current subtotal
        newDiscountAmount = Math.min(Number(state.orderSummary.subtotal || 0), Number(newDiscountAmount || 0));
        state.discountCoupon.discountAmount = newDiscountAmount;
        state.orderSummary.discount = newDiscountAmount;
      }
      state.orderSummary.total = calculateTotal(state.orderSummary);
    },
...
export const selectShippingTypeFormatted = (state) => state.checkout.shippingTypeFormatted;
...
export const selectPaymentPayload = createSelector(
  [(state) => state.checkout, (state) => state.cart, (state) => state.auth, selectSimplifiedCartProducts],
  (checkout, cart, auth, simplifiedCartProducts) => {
    ...
    return {
      ...
      // Shipping type formatted
      shipping_type: checkout.shippingType || checkout.shippingTypeFormatted,
      ...
      shipping_amount: (() => {
        const val = parseFloat(checkout.orderSummary.shipping || 0);
      ...
      // Shipping weight (using cart selector for simplified items)
      ...(function () {
        try {
          const weight = calculateShippingWeight(simplifiedCartProducts || []);
          return {
            box: weight.box,
            total_pounds: Number(weight.totalPounds || 0),
          };
        } catch (e) {
          return { box: { size: '', grams: 0, dimensions: null }, total_pounds: 0 };
        }
      })(),
    };
  },
