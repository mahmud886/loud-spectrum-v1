...
export const selectPaymentPayload = (state) => {
  ...
  const regularProducts = cart.items
    ...
    .map((item) => {
      ...
      return {
        product: subProduct?._id || item.id, // Use subProduct ID for backend
        name: item.name || item.product?.name,
        sku:
          subProduct?.sku ||
          `${(item.name || item.product?.name || 'product').replace(/\s+/g, '-')}-${item.selectedVolume}`,
        price: (isNaN(price) ? 1 : Math.max(1, price)).toFixed(2), // String format
        quantity: isNaN(quantity) ? 1 : Math.max(1, quantity), // Ensure valid number
        tax: '0.00', // Default tax per item
        total: (isNaN(total) ? 1 : Math.max(1, total)).toFixed(2), // String format
      };
    });
  ...
  return {
    ...
    customer: state.auth?.user?.id || null, // Customer ID from auth state
